...
sim18 <- readRDS("params/params7.RDS") # I think I saved previous sim_optim under params7.RDS, so this sim18 is post optimisation


params19 <- calibrateBiomassCustom(sim18@params)

sim19 <- project(params19, t_max = 1000)

```

B.mur has a too high steady state, going to look at it manually



snapshot of size spectrum with the area where the data comes from


```{r}
plot_dat <- plotSpectra(sim18, return_data = T)

ggplot(plot_dat) +
  geom_line(aes(x = w, y = value, color = Species)) +
  scale_x_continuous(trans = "log10") +
  scale_y_continuous(trans = "log10") +
  annotate("rect", xmin=sim18@params@species_params$biomass_cutoffLow[1], xmax=sim18@params@species_params$biomass_cutoffHigh[1], ymin=0, ymax=Inf, alpha=0.2, fill="red") +
  scale_color_manual(values = sim18@params@linecolour)
...
#fixing D.ele
params20 <- params19
params20@species_params$R_max[1] <- params19@species_params$R_max[1]*3
params20@resource_params$kappa <- params19@resource_params$kappa *50
params20 <- setParams(params20)

sim20 <- project(params20, t_max = 1000)
plotBiomass(sim20)

getBiomass(sim19)[1000,]
getBiomass(sim20)[1000,]

plotBiomassObservedVsModelCustom(sim20)

params20@initial_n <- sim20@n[1000,,]

params21 <- params20
params21@species_params$R_max[2] <- params20@species_params$R_max[2]*3
params21@species_params$R_max[3] <- params20@species_params$R_max[3]*3
params21@species_params$R_max[4] <- params20@species_params$R_max[4]/3
params21@species_params$R_max[6] <- params20@species_params$R_max[6]*2
params21@species_params$R_max[7] <- params20@species_params$R_max[7]*1.5
params21@species_params$R_max[8] <- params20@species_params$R_max[8]/100
params21@species_params$R_max[9] <- params20@species_params$R_max[9]/1000000
params21@species_params$erepro[9] <- params20@species_params$erepro[9]*10


params21 <- setParams(params21)

sim21 <- project(params21, t_max = 200)
plotBiomass(sim21)
plotBiomassObservedVsModelCustom(sim21)
getBiomass(sim20)[1000,]
getBiomass(sim21)[1000,]



```

Hard to get D.ele and B.mur coexisting while having biomass close to the data


```{r}

params19 <- scaleModel(params = sim18@params,factor = .25)
plotBiomassObservedVsModelCustom(params19)
plotGrowthCurves(params19, species_panel = T)

sim19 <- project(params19)
plotBiomass(sim19)
plotBiomassObservedVsModelCustom(sim19)
plotGrowthCurves(sim19, species_panel = T)
plotDiet2(sim19)

params19@initial_n <- sim19@n[100,,]
params20 <- params19
params20@species_params$R_max[8] <- params19@species_params$R_max[8]/10
params20@species_params$R_max[9] <- params19@species_params$R_max[9]/10000000000
sim20 <- project(params20, t_max = 1000)
plotBiomass(sim20)
params20@initial_n <- sim20@n[1000,,]
sim20 <- project(params20)

saveRDS(sim20, file = "params/params8.RDS")

# trying to fit the growth of D.ele at least

params20 <- params19
params20@species_params$gamma[1] <- params19@species_params$gamma[1]/5
...
plotBiomass(sim20)
plotBiomassObservedVsModelCustom(sim20)
plotGrowthCurves(sim20, species_panel = T)
```

There is always something wrong so rerunning optim with more params
...
optim_result <- readRDS("params/optim_result_truncatedB3.RDS")
```
...
toc()
saveRDS(optim_result, file="params/optim_result_timeseriesShort.RDS")
```
